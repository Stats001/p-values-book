...
```{r}
anova.2way.data="Treatment,Litter,Response
Drug,Litter.1,4
Drug,Litter.1,8
Drug,Litter.2,11
Drug,Litter.2,12
...
anova.2way.data=read.table(textConnection(anova.2way.data), header=TRUE, sep=",")
```
...
```{r}
ggplot(anova.2way.data, aes(y=Response, x=Treatment)) + geom_point() + facet_wrap(~Litter) + 
...
```{r}
anova.2way.data.outlier="Treatment,Litter,Response
Drug,Litter.1,4
Drug,Litter.1,8
Drug,Litter.2,11
Drug,Litter.2,12
...
anova.2way.data.outlier=read.table(textConnection(anova.2way.data.outlier), header=TRUE, sep=",")
```
...
```{r}
ggplot(anova.2way.data.outlier, aes(y=Response, x=Treatment)) + geom_point() + facet_wrap(~Litter) +  
...
```{r}
lm.2way.outlier = lm(Response ~ Litter + Treatment, data= anova.2way.data.outlier)
...
```{r}
anova.2way.data.outlier.lmrobM = lmrobM(Response ~ Litter + Treatment, data = anova.2way.data.outlier)
...
```{r}
anova.2way.data.lmrobM = lmrobM(Response ~ Litter + Treatment, data = anova.2way.data) # control=cont)
...
```{r}
anova.2way.data.outlier.rfit = with(anova.2way.data.outlier, rfit(Response ~ Litter + Treatment))
