...
```{r}
with(pcr.data,
  boxplot(DeltaCT ~ Treatment, xlab="Treatment", ylab = "Delta CT", ylim=c(11,14)))
```
...
```{r}
# Equivalently, the t-test analysis can also be done using the R function lm(),
# which performs a linear regression.
...
# Only change: add Treatment -> Grams ~ Sex + Treatment
